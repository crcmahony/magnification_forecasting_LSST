...
from Fisher_plot_3_same_axes import fisher_grid
import numpy as np
...
F2 = np.loadtxt('/unix/atlas4/akorn/LSST/cosmosis/cosmosis/modules/euclid_ias/demos/thesis_results/clustering_gold/output/fisher_clustering_gold.txt')
F3 = np.loadtxt('/unix/atlas4/akorn/LSST/cosmosis/cosmosis/modules/euclid_ias/demos/thesis_results/clustering_gold/output/fisher_clustering_gold_old.txt')
covF = np.linalg.inv(F)
covF2 = np.linalg.inv(F2)
covF3 = np.linalg.inv(F3)

filename = 'clustering_clustering_gold_clustering_gold_old_cos_params.png'
...
red_line = mlines.Line2D([],[],color='r', label = 'Clustering Faint', lw = 2.0)
blue_line = mlines.Line2D([],[],color='b', label = 'Clustering Gold', lw = 2.0) #F2 is blue, largest covariance first
green_line = mlines.Line2D([],[],color='g', label = 'Clustering Gold Old', lw = 2.0) #F3 is green
...
if plot_cos_params == True and plot_HOD_params == True and plot_nz_params == True and plot_bias_params == True:
        ...
        fisher_grid(covF, covF2, covF3, fiducial, params=par)
        plt.legend(handles=[red_line, blue_line, green_line], bbox_to_anchor=(0.0,6.0), loc='upper right',borderaxespad=0.3,fontsize = 34)
        ...
        fisher_grid(covF[:7,:7], covF2[:7,:7], covF3[:7,:7], fiducial, params=par)
        plt.legend(handles=[red_line, blue_line, green_line], bbox_to_anchor=(0.0,6.0), loc='upper right',borderaxespad=0.3,fontsize = 34)
        ...
        fisher_grid(covF[7:16,7:16], covF2[7:16,7:16], covF3[7:16,7:16], fiducial, params=par)
        plt.legend(handles=[red_line, blue_line, green_line], bbox_to_anchor=(0.0,7.0), loc='upper right',borderaxespad=0.3,fontsize = 34)
